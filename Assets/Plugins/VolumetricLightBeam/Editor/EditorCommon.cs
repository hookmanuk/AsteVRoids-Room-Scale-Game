...
namespace VLB
{
    public class EditorCommon : Editor
    {
        ...
        SerializedProperty FindProperty(string name)
        {
            ...
            if (prop == null)
            {
                name = string.Format("_{0}{1}", char.ToUpperInvariant(name[0]), name.Substring(1)); // get '_Name' from 'name'
                prop = serializedObject.FindProperty(name);
            }
            return prop;
        }

        void RetrieveSerializedProperties(Type t)
        {
            if (t == null) return;
            ...
            foreach (var field in allEditorFields)
            {
                if (field.FieldType == typeof(SerializedProperty))
                {
                    ...
                    var serializedProp = FindProperty(runtimeFieldName);
            ...
            RetrieveSerializedProperties(t.BaseType);
        }

        protected void RetrieveSerializedProperties()
        {
            RetrieveSerializedProperties(GetType());
        }
